...
cleansight-pipeline/
  ...
  utils/            # 各脚本共用工具(不单独运行)
    ...
    labelstudio.py  # LS 解析核心:fps 对齐、关键帧插值、坐标转换
  ...
  # ---- 跨数据集编排 ----
  common/
    reconcile.py         # 对账/增量前置:列出待办;--assign 回填 split
    pull.py              # 从 LS 下视频到 raw/videos/ + 完整性抽查
    check.py             # 推送前校验(yolo)
    upload_git.py        # 统一 git 版:一键把两套数据集全传 ModelScope(--dry-run)
    count_classes.py     # 跨数据集的类别计数报表
    scarce_sources.py    # 解析导出,建立 task→视频→类 映射
    scarce_checklist.py  # 稀缺类补采/补标清单

  # ---- 数据集:yolo(检测)----
  yolo/
    build.py              # 转 YOLO,按 splits.yaml 整段路由,打印样本分布
    augment.py            # 稀有类别数据增强(备用,仅 train)
    train.py              # 各组训练
    validate.py           # 验证集指标 + 验收判定 + 报告
  ...
  actionmixed/
    build.py   # bbox+动作同存;段级内存哈希切分(不碰 splits.yaml)
...
| 部分 | 定位 | 何时用 |
|------|------|--------|
| `common/reconcile.py` | **对账中枢**:比对"导出/磁盘/splits/白名单"四方,列出待办;`--assign` 回填 split | 每次开工、每次增量前先跑 |
| `common/pull.py` | **取数**:按导出 JSON 从 LS 下视频到 `raw/videos/` + 完整性抽查 | 有"未下载"视频时 |
| `common/check.py` | **推送前校验**:扫 yolo 产物,不达标拒推 | 上传前 |
| `common/upload_git.py` | **一键全传**:git 方式把两套数据集推 ModelScope | 两套都就绪、要统一发布时 |
| `yolo/build.py` | yolo 转换:导出+视频 → 分组 YOLO 集,按 `splits.yaml` 整段路由 | 数据/切分变化后重建 |
| `yolo/train.py` · `yolo/validate.py` | yolo 训练 / 验收(阈值门禁,FAIL 退出码非零) | 出/更新权重、交付卡口 |
...
自带虚拟环境,不复用外部 venv。数据阶段(`pull`/`reconcile`/`build`)只需 `cv2`、`numpy`、`pyyaml`;训练/验证(`train`/`validate`)另需 `ultralytics`(含 torch);`upload.py` 需 `modelscope`。
...
.venv/bin/python common/pull.py     # 1. 下视频到 raw/videos/
.venv/bin/python common/reconcile.py        # 2. 看对账;质检合格的加进 config.only_videos
.venv/bin/python common/reconcile.py --assign  # 3. 给已质检视频确定性回填 split(写回 splits.yaml,提交它)
.venv/bin/python yolo/build.py           # 4a. yolo 检测集 → datasets/
.venv/bin/python actionmixed/build.py    # 4b. 动作识别集 → datasets_actionmixed/
```

各 `build.py` 会打印逐 split × 逐类的帧数/框数,并对"每视频尾部覆盖 < 80%""某类 val 无样本"等给出告警——务必扫一眼。样本分布可随时独立重算:`.venv/bin/python common/count_classes.py`。

### 场景二:训练与评估(仅 yolo)

```bash
.venv/bin/python yolo/train.py           # 各组训练;权重落 runs/<组>/weights/best.pt
.venv/bin/python yolo/validate.py        # 验证集指标 + 验收报告 runs/<组>/acceptance_report.md
```
...
.venv/bin/python common/reconcile.py           # 看差异:未下载/未质检/未归属/遗失/孤儿
.venv/bin/python common/pull.py        # 补下"未下载"
# 人工质检合格的,追加到 config.yaml 的 only_videos
.venv/bin/python common/reconcile.py --assign  # 回填"未归属"(已有视频 split 不变 → 天然增量)
.venv/bin/python yolo/build.py              # 重建(增量跳过已完成任务)
.venv/bin/python actionmixed/build.py
.venv/bin/python yolo/validate.py           # (如重训了)重新验收
```

`common/reconcile.py` 的分类与动作:

| 分类 | 含义 | 该做什么 |
|------|------|---------|
| 未下载 | 导出引用了但磁盘没有 | 跑 `common/pull.py` |
| 未质检 | 已下载但不在 `only_videos` | 人工质检后追加到 `config.only_videos` |
| 未归属 | 已质检但 `splits.yaml` 无 split | 跑 `common/reconcile.py --assign` |
...
- **视频**:存 LS 服务器,`common/pull.py` 下到 `raw/videos/`;**身份 = 文件名 stem**(如 `687e3c78-clip_<起>_<止>`)。
- **标注**:LS 导出 JSON 放 `raw/exports/`,脚本取**文件名排序最后一份**。同一份导出里 bbox(`videorectangle`)与时序(`timelinelabels`)聚合,两套 build 各取所需。
- 视频与标注靠 `task.data.video` 的文件名关联;`common/reconcile.py` 对齐"导出/磁盘/splits/白名单"四方。
...
- 回填是**显式步骤**(`--assign`),不在 `build` 里静默改动;`build` 遇未归属视频报错提示(或 `--auto-assign` 当场回填)。
...
`yolo/validate.py` 在**验证集**上跑 `ultralytics.val`,逐类 P/R/mAP 对照 `config.acceptance` 判 PASS/FAIL,写 `runs/<组>/acceptance_report.md`。默认门槛(在 `config.yaml` 调):整体 mAP@0.5 ≥ 0.5、mAP@0.5:0.95 ≥ 0.3、逐类 recall ≥ 0.7、precision ≥ 0.5、每类都要有验证样本。
