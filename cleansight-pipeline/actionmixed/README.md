...
**bbox + 动作标签同存**:每帧带 YOLO 检测框(`frames/`),每视频带动作标签(`labels/`),动作段前后各扩展 idle 帧。所有命令在上级 `cleansight-pipeline/` 下执行。

## 脚本
| 脚本 | 作用 |
|------|------|
| `build.py` | 导出+视频 → `datasets_actionmixed/{images,frames,labels}`;`--force` 全量重建 |
| `upload.py` | SDK 上传 → `cleansight-ActionMixed` |

## State(入库)
- `completed_tasks_actionmixed.json` — 增量完成清单
- `tracking_actionmixed.md` — `build.py` 生成的追踪表
